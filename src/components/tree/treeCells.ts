/**
 * Heatmap tree — silhouette sampling.
 *
 ...
 * import paths rewritten and the local PRNG swapped for the project's shared
 * `createRng` (the same mulberry32). The algorithm is theirs.
 *
 * How it works: the silhouette PNG is drawn into an offscreen canvas at one
 * pixel per grid cell, and every cell whose alpha clears the threshold becomes
 * a tree cell. Intensity comes from how far a cell sits from the silhouette's
 * edge (thickness) plus a directional light, so the trunk reads solid and the
 * twigs read faint. Cells also record their distance from the root, which is
 * what the renderer grows along.
 */
import { CELL_SIZE, type TreeAnchor } from './treeConstants';
