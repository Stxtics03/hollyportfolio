 ...
 * template string, and reduced-motion read from this project's hook rather
 * than a module-level helper. The palette, the sway modes and the growth
 * behaviour are the original author's.
 */
import { useEffect, useRef } from "react";

import { buildTreeCells, SILHOUETTE_SRC, type LeafGroupBase } from './treeCells';
import {
  CELL_SIZE,
  GROWTH_MS,
  SQUARE_SIZE,
...
export default function HeatmapTree({
  ...
  }, [showGrid, transparent, anchor, fillFactor, illumBoost, illumDim, composed, reducedMotion]);

  return (
    <canvas
      ref={canvasRef}
      className={`block h-full w-full ${transparent ? 'bg-transparent' : 'bg-black'} ${className}`}
...
export type { TreeAnchor } from './treeConstants';
